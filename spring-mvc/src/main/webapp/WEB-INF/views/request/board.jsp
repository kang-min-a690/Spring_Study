	...
	<form action="<%= request.getContextPath() %>/request/board " method="post" >
		<input type="text" name="no" placeholder="번호입력"/>
		<input type="submit" value="등록"/>
	</form>
	
	<h3> PUT 방식으로 요청 </h3>
	<form action="<%= request.getContextPath() %>/request/board " method="post" >
		<!-- form 태그 에서는 PUT 방식 요청을 지원하지 않는다 -->
		<!-- Spring 의 HiddenMethodFilter 를 등록하면 , 우회적으로  PUT 요철 할수 있다 -->
		<!-- _method 를 X-HTTP-Method-Override 헤더로 지정 한다. -->
	...
	<hr>
	<h3> POST 방식으로 요청 - /request/body </h3>
